...
test('collects rewards freely and lets a parent redeem them later', async ({ page }) => {
  ...
  await page.reload()
  await expect(page.getByText('Eingelöst')).toBeVisible()
  await page.getByRole('button', { name: /Einlösen rückgängig machen/ }).click()
  await page.getByLabel('PIN', { exact: true }).fill('2468')
  await page.getByRole('button', { name: 'Rückgängig machen', exact: true }).click()
  await expect(page.getByText('Bereit zum Einlösen')).toBeVisible()
  await expect(page.getByText('Eingelöst')).not.toBeVisible()
  await page.reload()
  await expect(page.getByText('Bereit zum Einlösen')).toBeVisible()
  await page.getByRole('button', { name: 'Zurück zum Start' }).click()
  ...
  await expect(page.getByRole('heading', { name: 'Wirklich alle Daten löschen?' })).toBeVisible()
  await page.getByRole('button', { name: 'Jetzt alles zurücksetzen' }).click()
  await expect(page.getByRole('heading', { name: 'Mathe-Mission' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Belohnung wählen' })).toBeVisible()
  await page.reload()
  await expect(page.getByRole('button', { name: 'Belohnung wählen' })).toBeVisible()
  await page.getByRole('button', { name: 'Elternbereich' }).click()
  await expect(page.getByRole('heading', { name: 'Eltern-PIN einrichten' })).toBeVisible()
})

test('an adult controls manual and automatic task reading', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: 'Belohnung wählen' }).click()
  await chooseReward(page)
  await chooseMission(page)
  await expect(page.getByRole('button', { name: 'Aufgabe vorlesen' })).toHaveCount(0)

  await page.getByRole('button', { name: 'Mission verlassen' }).click()
  const closePwaNotice = page.getByRole('button', { name: 'Hinweis schliessen' })
  if (await closePwaNotice.isVisible()) await closePwaNotice.click()
  await page.getByRole('button', { name: 'Elternbereich' }).click()
  await page.getByLabel('Neue PIN', { exact: true }).fill('2468')
  await page.getByLabel('PIN wiederholen').fill('2468')
  await page.getByRole('button', { name: 'PIN speichern' }).click()

  const speechAllowed = page.getByRole('checkbox', { name: /^Vorlesen erlauben/ })
  const automaticReading = page.getByRole('checkbox', { name: /^Automatisch vorlesen/ })
  await expect(speechAllowed).not.toBeChecked()
  await expect(automaticReading).toBeDisabled()
  await speechAllowed.check()
  await expect(automaticReading).toBeEnabled()
  await automaticReading.check()
  await page.getByRole('button', { name: 'Einstellungen speichern' }).click()

  await page.getByRole('button', { name: 'Zurück zum Start' }).click()
  await page.getByRole('button', { name: 'Mission starten' }).click()
  await expect(page.getByRole('button', { name: 'Aufgabe vorlesen' })).toBeVisible()
  ...
  await page.getByLabel('PIN', { exact: true }).fill('2468')
  await page.getByRole('button', { name: 'Öffnen' }).click()
  await page.getByRole('checkbox', { name: /^Vorlesen erlauben/ }).uncheck()
  await expect(page.getByRole('checkbox', { name: /^Automatisch vorlesen/ })).toBeDisabled()
  await expect(page.getByRole('checkbox', { name: /^Automatisch vorlesen/ })).not.toBeChecked()
  await page.getByRole('button', { name: 'Einstellungen speichern' }).click()
