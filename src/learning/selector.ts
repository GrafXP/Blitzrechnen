...
export function challengeForSession(
  ...
  const due = SKILLS.filter((skill) => isDue(data.mastery[skill.id], dateKey))
  const currentTopic = SKILLS.filter((skill) => skill.topics.includes(data.settings.schoolTopic))
  const roll = random()
  let pool = roll < 0.5 ? due : roll < 0.8 ? currentTopic : SKILLS
